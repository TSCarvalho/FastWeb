...
SEARCHLAYOUT
        ...
        private function general_admin_messages() {
            $smart_options['log_schedule_messages'] = array( 'type'    => 'checkbox', 'default' => '1', );
		...
		private function get_string_default( $key ) {
			...
			$text_to_return = $this->slplus->Text->get_text_string( array( 'option_default', $key ) );
		...
		private function set_valid_option( $val, $key, $which_option ) {
			...
			if ( $value_is_empty ) {
				$default_value = $is_smart_option ? $this->$key->default : $this->slplus->{$default_option}[ $key ];
