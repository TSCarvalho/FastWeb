...
if ( ! class_exists( 'SLP_Settings' ) ) {
	...
	require_once( SLPLUS_PLUGINDIR . 'include/module/admin_tabs/settings/SLP_Settings_Section.php' );
	 ...
	 *
	 * @property        string[]               $definition_directories  array of directory paths where setting definitions might live
	...
	class SLP_Settings extends SLPlus_BaseClass_Object {
		...
		public $definition_directories = array();
		...
		protected function initialize() {
			 ...
			 *
			 * @filter     slp_setting_definition_directory
			 *
			 * @params     string[]      list of directories to search for a setting definition
			 *
			 * @return     string[]      augmented/replaced directory list
			 */
			$this->definition_directories = apply_filters( 'slp_setting_definition_directory' , array( SLPLUS_PLUGINDIR . 'include/module/admin_tabs/settings/' ) );
                                        ...
                                        <ul class="sub-navbar">
                                            ...
                                            foreach ( $this->sections as $section ) {
                                                if ( $section->auto ) {
                                                    ...
                                                    $firstClass   = ( ( "#wpcsl-option-{$friendlyDiv}" == $selectedNav ) || ( $firstOne && ( $selectedNav == '' ) ) ) ? ' first current open' : '';
                                                    $firstOne     = false;

                                                    $link_id = "wpcsl-option-{$friendlyDiv}";
                                                    print "<li class='top-level general {$firstClass} navbar-item'>" . sprintf( "<a id='%s_sidemenu' class='navbar-link subtab_link' data-slug='%s' href='#%s' title='%s' >%s</a>" , $link_id , $friendlyDiv , $link_id , $section->name , $section->name ) . '</li>';
                                                }
                                            }
