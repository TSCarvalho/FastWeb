...
if ( ! class_exists( 'SLP_BaseClass_Admin' ) ) {
    ...
    class SLP_BaseClass_Admin extends SLP_Object_With_Objects {
        ...
        private function nq_this_file( $hook , $css_file , $augmentation = '' ) {
		    ...
		    wp_enqueue_style( $augmentation . $this->addon->short_slug . '_admin_css', $this->addon->url . '/' . $css_file );
        ...
        function enqueue_admin_javascript( $hook ) {
	        ...
	        switch ( $hook ) {
		        ...
		        case $this->slplus->admin_page_prefix . 'slp_experience':
		        	$files= array( 'js/admin-experience-tab.min.js' , 'js/admin-experience-tab.js' );
			        break;
	        }
	        ...
	        if ( ! empty ( $files ) ) {
		        foreach ( $files as $file ) {
			        if ( file_exists( $this->addon->dir . $file ) ) {
			        	$slug_suffix = str_replace( $this->slplus->admin_page_prefix , '_' , $hook );
				        wp_enqueue_script( $this->addon->slug . $slug_suffix , $this->addon->url . '/' . $file , $this->js_requirements );
				        break;
			        }
		        }
	        }
	     ...
	     *
	     * TODO: Move to base_class.admin.php and rewire calls from add on packs afterwards (ELM, POWER, PRO, SME, UML)
	     *
