 ...
 *
 * @property-read   SLP_Admin_Settings_Text $Admin_Experience_Text
...
class SLP_Admin_Settings_Tab extends SLP_Object_With_Objects {
	...
	protected $objects = array(
		'Admin_Settings_Text' => array( 'auto_instantiate' => true, 'subdir' => 'include/module/admin_tabs/' ),
		'Settings' => array( 'auto_instantiate' => true, 'subdir' => 'include/module/admin_tabs/settings/'),
	);
	...
	function add_view() {
		...
		$theme_list = $this->slplus->Plugin_Style->get_theme_list();
		...
		$this->Settings->add_ItemToGroup( array(
			'group_params' => $this->group_params,
			'label'        => '',
			'description'  => $this->slplus->Plugin_Style->setup_ThemeDetails( $theme_list ),
			'setting'      => 'themedesc',
			'type'         => 'subheader',
		) );
	}
	 ...
	 */
	private function create_checkbox_post( $checkbox_name ) {
	...
	function save_options() {
		 ...
		 *
		 * TODO: used by EXP  , can this be done another way through the base class admin_startup() maybe?
		 */
	 ...
	 *
	 * TODO: Remove once the filter slp_save_map_settings_inputs is gone.
	 *
	 * @used-by SLP_Experience_Admin::add_hooks_and_filters()
	 */
	private function SavePostToOptionsTable( $optionname, $default = null, $cboptions = null ) {
		if ( $default != null ) {
