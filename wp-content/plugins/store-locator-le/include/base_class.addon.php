...
if (!class_exists('SLP_BaseClass_Addon')) {
    ...
    class SLP_BaseClass_Addon extends SLP_Object_WithOptions {
	    ...
	    public function auto_load( $class_name ) {
		    if ( strpos( $class_name , 'SLP_' ) === false ) {
			    return;
		    }


		    // And SLP Base Class or an Add On Class
		    //
		    if ( strpos( $class_name , 'SLP_Base_' ) === false ) {
		    ...
		    if ( array_key_exists( $object_name , $this->slplus->objects ) ) {
			    return;
		    }
	    ...
	    private function check_my_version_compatibility() {
		    if ( isset( $this->slplus->min_add_on_versions[ $this->short_slug ] ) && version_compare( $this->version , $this->slplus->min_add_on_versions[ $this->short_slug ] , '<' ) ) {
			    ...
			    $this->slplus->Helper->add_wp_admin_notification( sprintf( __( '%s version %s has been disabled. ' , 'store-locator-le' ) , $this->name , $this->version ) . '<br/>' . sprintf( __( 'It is not compatible with version %s of Store Locator Plus. ' , 'store-locator-le' ) , SLPLUS_VERSION ) . '<br/>' . $this->slplus->Text->get_web_link( 'check_website_for_upgrades' ) . '<br/>' . $this->slplus->Text->get_web_link( 'new_addon_versions' ) );
	    ...
	    function create_object_Updates( $force ) {
		    ...
		    if ( ! isset( $this->Updates ) ) {
			    require_once( SLPLUS_PLUGINDIR . 'include/module/internet/SLP_Updates.php' );
			    $this->Updates = new SLP_Updates();
			    if ( ! $force ) {
	    ...
	    function createobject_Admin() {
		    if ( ! isset( $this->admin ) ) {
		    	...
		    	if ( is_readable( $this->dir . 'include/class.admin.php' ) ) {
				    ...
				    $this->admin = new $this->admin_class_name( [ 'addon' => $this ] );
					return;
			    }
	    ...
	    function createobject_AJAX() {
		    if ( ! isset( $this->ajax ) ) {
			    ...
			    if ( is_readable( $this->dir . 'include/class.ajax.php' ) ) {
				    ...
				    $this->ajax = new $this->ajax_class_name( [ 'addon' => $this ] );
				    return;
			    }
	     ...
	     */
	    public function create_object_option_manager() {
	     ...
	     * @used-by \SLPPower::modify_page_title
	     */
	    public function createobject_UserInterface() {
		    if ( ! isset( $this->userinterface ) ) {
			    ...
			    if ( is_readable( $this->dir . 'include/class.userinterface.php' ) ) {
				    ...
				    $this->userinterface = new $this->userinterface_class_name( [ 'addon' => $this , 'slplus' => $this->slplus ] );
				    return;
			    }

			    // autoload version
			    if ( class_exists( $this->userinterface_class_name ) ) {
				    $this->userinterface = new $this->userinterface_class_name( [ 'addon' => $this ] );
			    } else {
	    ...
	    private function create_object_widget() {
		    if ( ! isset( $this->widget ) ) {
			    ...
			    $this->widget = new $this->widget_class_name( [ 'addon' => $this ] );
		    }
	    }
	    ...
	    private function register_addon( $slug , $object ) {
		    $this->slplus->AddOns->register( $slug , $object );
	    }
