...
class WC_Shipping {
	...
	public function load_shipping_methods( $package = array() ) {
		if ( ! empty( $package ) ) {
			...
			// Debug output
			if ( $debug_mode && ! defined( 'WOOCOMMERCE_CHECKOUT' ) && ! defined( 'WC_DOING_AJAX' ) && ! wc_has_notice( 'Você está no "' . $shipping_zone->get_zone_name() . '"' ) ) {
				wc_add_notice( 'Você está no "' . $shipping_zone->get_zone_name() . '"' );
			}
		} else {
